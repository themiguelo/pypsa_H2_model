...
def data_frame ():
    ...
    technology_parameters.to_excel(os.path.join("pypsa_H2_model/my_pypsa_model/Data", "technology_parameters_data_frame_sensibility_analysis.xlsx"))
    ...
    technology_parameters.to_excel(os.path.join("pypsa_H2_model/my_pypsa_model/Data", "technology_parameters_data_frame_annualized_sensibility_analysis.xlsx"))
...
def setup_model(time_series_cf, technology_parameters, load, operation_series_max, operation_series_min, x, scenario_id, sensibility, value):
    ...
    if scenario_id in [2,3,5]:
     if sensibility == 2:
        n.add(
            ...
            capital_cost=technology_parameters.at["pv", "capital_cost"]*value,
    ...
    if scenario_id in [1,3,4]:
     if sensibility == 1:
        n.add(
            ...
            capital_cost=technology_parameters.at["on_wind", "capital_cost"]*value,
    ...
    if scenario_id in [4,5,6]:
     if sensibility == 4:
        n.add("Generator",
            ...
            marginal_cost=technology_parameters.at["grid", "VOM"]*value,
            efficiency=1,
            p_nom_extendable=True,
            )
     else:
    ...
    if scenario_id in [1,2,3,5]:
     if sensibility == 3:
        n.add("StorageUnit",
            ...
            capital_cost=technology_parameters.at["battery", "capital_cost"]*value,
    ...
    if sensibility == 5:
        n.add("Link",
                ...
                #bus2="electricity",
                carrier="hydrogen",
                capital_cost=technology_parameters.at["electrolyser_PEM", "capital_cost"]*value,
...
####################--------------- 4.0 Save & Load Model---------------###################
def save_network_to_netcdf(n, total_load_kg, scenario_id, sensibility, value):
    #saves the PyPSA network to a NetCDF file. 
    output_dir = f"results_sensibilities/scenario_{scenario_id}"
    ...
    filename = os.path.join(output_dir, f"network_model_scenario_{scenario_id}_{sensibility}_{value}.nc")
    ...
    for scenario_id in scenario_ids:
        for sensibility in sensibility_cases:
            for value in sensibility_values:
                print(f"\n================= Running Scenario {scenario_id}_{sensibility}_{value} =================")
                time_series_cf, tech_params, load, total_load_kg, op_max, op_min = data_frame()
                x = compressor_ratio()
                n = setup_model(time_series_cf, tech_params, load, op_max, op_min, x, scenario_id, sensibility, value)
                optimize(n)
                save_network_to_netcdf(n, total_load_kg, scenario_id, sensibility, value)
